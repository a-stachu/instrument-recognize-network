...
for segment in range(len(train_files_path)):
    file_path = train_files_path[segment]
    melspectrogram = np.load(file_path)
    train_files.append(melspectrogram)

for file in range(len(os.listdir(train_path))):
    train_files_labels.append(prepare_labels(file)[0])  # family 0 inst 1

train_files_labels = [label for row in train_files_labels for label in row]
